...
export default function HistoryPage() {
  ...
  const handleDownloadPDF = (idx, topic) => {
    ...
    if (isCompare) {
      const combinedText = 
        '# Single Agent Report\n\n' + (result.single?.report || '') +
        '\n\n---\n\n# Multi-Agent Report\n\n' + (result.multi?.report || '');
      downloadReportPDF(combinedText, `${topic || 'Research'} Comparison`);
    } else {
      downloadReportPDF(result.report || '', `${topic || 'Research'} Report`);
    }
  };
